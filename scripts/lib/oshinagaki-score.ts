 ...
 * 足切りは現在ひとつも無い。
 *
 * かつて「浜松に言及し、大阪・東京に触れていない投稿」をスコア0にしていた。
 * 会場帰属の判定がまだ無く、スコアだけで掲載を決めていた頃の名残である。
 *
 * いまは掲載条件が
 *   会場が証明されている（attribution）× お品書き/商品である（curation）
 * の二段構えになっており、浜松専用の投稿は「大阪と証明できない」ので
 * 自動的に載らない。足切りは不要になった。
 *
 * むしろ有害だった。実データ:
 *   「【鬱P新譜情報】マジカルミライクリエイターズマーケットにて新譜カセット
 *     テープ「H.M.1996」を頒布します。1000円です。…🚨浜松は26日(日)のみなので要注意！」
 * これは3会場すべての新譜情報だが、末尾の注意書きだけで浜松専用と誤判定され、
 * スコア0になって候補にすら入らなかった。
 * さらにスコア0だと画像判別の対象からも外れ、拾い直す機会も失われる。
 */
