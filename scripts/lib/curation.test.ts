...
test('企業ブースの「販売商品情報」は価格が本文に無くても載せる', () => {
  // 企業は価格を画像に書く。本文に価格を要求していたせいで、会場も
  // ブース番号も確定している商品案内が軒並み落ちていた（実測130件）。
  for (const text of [
    '【#初音ミク「#マジカルミライ2026」】＼大阪会場(ブース:D10)販売商品情報／' +
      ' 初音ミク「マジカルミライ 2026」のメインビジュアルを使用したグッズが登場',
    '〈初音ミク「マジカルミライ 2026」〉COSPAグッズ情報公開 緜氏が描く初音ミクを大胆にデザイン',
    '【新譜情報】オリジナルEP with 鏡音リン「ASTEROID」できました。マジカルミライ2026大阪・東京',
  ]) {
    assert.equal(isProductPost(post({ id: '1', text })), true, `落ちている: ${text.slice(0, 24)}`);
  }
});

test('「メニューです」も頒布物の一覧として扱う', () => {
  const p = post({
    id: '1',
    text:
      'マジカルミライ2026 OSAKA @インテックス大阪 8/15.16（土日）C-10「お商材屋さん」メニューです',
  });
  assert.equal(isProductPost(p), true);
});

test('価格が無くても「何を持っていくか」が並んでいれば載せる', () => {
  const p = post({
    id: '1',
    text:
      '【参加】来週8/14-16はマジカルミライ大阪 クリエイターズマーケットA-7にいます' +
      ' 新譜『OTONA DAIGAKU』旧譜各種 缶バッジとステッカーがあります',
  });
  assert.equal(isProductPost(p), true);
});

test('値段と頒布の明示があれば、決まった語彙が無くても載せる', () => {
  // 語彙の一覧（新譜・新商品…）に無いというだけで落ちていた実データ。
  // 何がいくらで買えるかは書いてある。
  for (const text of [
    'マジカルミライ2026 TOKYO クリエイターズマーケット B-2 ツムギ食堂にて' +
      '「kawaiiはつくれる！！おかわり！！」頒布します！5曲入り1000円！',
...
test('品目が行を分けて並んでいても拾う', () => {
  // 判定の窓が改行をまたげず、この形が一件も拾えていなかった。
  const p = post({
    id: '1',
    text: '【参加】来週8/14-16はマジカルミライ大阪 クリエイターズマーケットA-7にいます\n新譜『OTONA DAIGAKU』\n旧譜各種\n缶バッジとステッカーがあります',
  });
  assert.equal(isProductPost(p), true);
});

test('「完売する可能性があります」は振り返りではない', () => {
  // 本文に「お品書きです」と明記され会場も確定しているのに、
  // 末尾の注意書きだけで振り返り扱いになって落ちていた。
  const p = post({
    id: '1',
    text:
      '8/14-16、マジカルミライ大阪・クリエイターズマーケットのお品書きです。' +
      '新作「革命ノ赤」含む5種（CD3、SONOCA2）の頒布となります。' +
      '※旧譜CDは1-2日目までに完売となる可能性があります。ご了承ください。',
  });
  assert.equal(isOshinagakiPost(p), true);
});

test('前の会場で売り切れたが再入荷した話は振り返りにしない', () => {
  // 本文に「大阪のお品書きです」と明記され会場も確定しているのに、
  // 「売り切れ」の一語で落ちていた（たきだしごはん 大阪D-1）。
  const p = post({
    id: '1',
    text:
      'マジカルミライ大阪のお品書きです！\n' +
      '新たに「青炎」刺繍ブレスレットがあります！\n' +
      '星の音も浜松で売り切れてしまったのですが、再入荷しました！あります！',
  });
  assert.equal(isOshinagakiPost(p), true);
});

test('打ち消しは同じ文の中だけを見る', () => {
  // 「ありがとうございました」と「あります」が離れた文にある場合、
  // 打ち消しとして使わない（お礼の投稿を通してしまわないように）
  const p = post({
    id: '1',
    text: 'マジカルミライ浜松ありがとうございました。\nお品書きはこちらでした。\n次もあります',
  });
  assert.equal(isOshinagakiPost(p), false);
});

test('前の会場での完売に触れた次の会場のお品書きは落とさない', () => {
  // 「お品書きを公開」と名乗っているのに、先週の大阪の完売に触れただけで
  // 振り返り扱いになり、東京のお品書きが丸ごと落ちていた（ろいどる！ 東京C-17）。
  const p = post({
    id: '1',
    text: [
      '#マジカルミライ2026 TOKYO',
      'クリエイターズマーケットのお品書きを公開！',
      'esoragoto曲のミクver.なども収録されたCHAPTER#3と、アクスタを新発売します！',
...
test('お品書きを名乗っていても、お礼や撤収があれば振り返りのまま', () => {
  // 完売の扱いを緩めたぶん、イベントが終わった報告まで通してはいけない。
  for (const text of [
    'マジカルミライ大阪のお品書きです。3日間ありがとうございました！完売しました',
    'お品書きを公開していましたが、撤収しました。新譜は売り切れです',
  ]) {
    assert.equal(isOshinagakiPost(post({ id: '1', text })), false, `通っている: ${text}`);
  }
});

test('見出しが「お品書き」の投稿は、完売に触れていても落とさない', () => {
  // 「【C-1】お品書き」で改行して品目が続く形。「です」も「公開」も無いので
  // 言い切りの形だけを見ていると、末尾の「売り切れ」で落ちていた（のん・木瀬 東京C-1）。
  const p = post({
    id: '1',
    text: [
      '#マジカルミライ2026 東京',
...
test('実際に完売した報告は従来どおり落とす', () => {
  for (const text of [
    'マジカルミライ2026 浜松 完売しました！ありがとうございました',
    'お品書きの新譜は売り切れました',
  ]) {
    assert.equal(isOshinagakiPost(post({ id: '1', text })), false, `通っている: ${text}`);
  }
...
test('「新譜だします」も何を売るかの表明として拾う', () => {
  // 会場もブース番号も公式と一致しているのに、動詞が一覧に無いだけで
  // 落ちていた（ヤデュクシチャンネル 大阪A-6 / 東京B-18）。
  for (const text of [
    '【#マジカルミライ2026 】クリエイターズマーケットに出ます！' +
      '◆8/14～16 大阪 A-6 ◆8/28～30 東京 B-18 巡音ルカの新譜だします！がんばります！',
    'マジカルミライ2026 クリエイターズマーケット 新刊を出します！',
  ]) {
    assert.equal(isProductPost(post({ id: '1', text })), true, `落ちている: ${text.slice(0, 24)}`);
  }
});

test('後日なのが通販の話だけなら予告として弾かない', () => {
  const p = post({
    id: '1',
    text:
      '「レイニースノードロップ」のグッズが #マジカルミライ2026 大阪、東京のCRECOブースにて先行販売' +
      ' ぬいぐるみを追加した事後通販（受注販売）の詳細は後日公開予定！',
  });
  assert.equal(isProductPost(p), true);
});

test('中身が何も無い予告は従来どおり落とす', () => {
  const p = post({
    id: '1',
    text: '特別な新譜が完成しました マジカルミライ東京で頒布します！！ 詳細は後日',
  });
  assert.equal(isProductPost(p), false);
});

test('会場からの実況は商品紹介にしない', () => {
  // 「グッズ…販売」が揃うので、実況を弾かないと素通りする。
  // 終了済みの浜松からの実況が大阪・東京のページに並んでしまう。
  for (const text of [
    '「マジカルミライ 2026」浜松最終日 初音ミクシンフォニーブース出展中' +
      ' 初音ミクシンフォニー2026札幌、東京公演公式グッズを販売',
    'マジカルミライ 2026 HAMAMATSU 2日目 開場しました COSPAブースでグッズを販売しています',
  ]) {
    assert.equal(isProductPost(post({ id: '1', text })), false, `通っている: ${text.slice(0, 24)}`);
  }
});
