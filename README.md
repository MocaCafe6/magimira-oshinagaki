...
「お品書きだと名乗っている」形は2つある。言い切り（「お品書きです」「お品書きを
公開」）と、**見出し**（行末が「お品書き」で次の行から品目が並ぶ）。後者を
見落としていたため、次の形が落ちていた（のん・木瀬 東京C-1）:

```
#マジカルミライ2026 東京
木瀬のんブース【C-1】お品書き        ← 「です」も「公開」も無い

・2nd Album『PIANØISM』…
音街ウナステッカーは売り切れてしまい在庫なしです🙏   ← これで振り返り扱い
```
...
npm run fetch-images-for-review -- --out .scan --ids "$IDS" --limit 400 --all-media
...
**`--all-media` を付ける。** fetch-images-for-review は既定で投稿の1枚目しか
落とさないが、**お品書きが2枚目以降にある投稿がある**。実データ:

| 投稿 | 1枚目 | 2枚目以降 |
|---|---|---|
| PERIHAPI! 東京E2 | 出展告知のバナー（価格なし） | Single ¥1,870 / Box ¥14,960 の価格表 |
| たきだしごはん 東京D-1 | 「D-1 たきだしごはん」（会場名なし） | 「マジカルミライ2026 **TOKYO** D-1」 |

1枚目だけを見ていると、前者は「ただの告知」に、後者は「会場が分からない」に
見える。どちらも2枚目を見れば片付く。
